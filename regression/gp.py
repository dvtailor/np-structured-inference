...
def main():
    ...
    # OOD settings
    parser.add_argument('--eval_kernel', type=str, default=None)
    ...
    if args.expid is None:
        if (args.mode == 'train') & (not args.resume):
            ...
            if args.min_num_cavi_steps < args.max_num_cavi_steps:
                args.expid += '_{}'.format(args.min_num_cavi_steps)
        else:
            args.expid = 'default'

    if args.expid is not None:
        args.root = osp.join(results_path, 'gp', args.train_kernel, args.model, args.expid)
    else:
        args.root = osp.join(results_path, 'gp', args.train_kernel, args.model)

    if args.eval_kernel is None:
        args.eval_kernel = args.train_kernel
    
    model_cls = getattr(load_module(f'models/{args.model}.py'), args.model.upper())
    config_type = 'mixsine' if args.train_kernel == 'mixsine' else 'gp'
    with open(f'configs/{config_type}/{args.model}.yaml', 'r') as f:
        config = yaml.safe_load(f)
    # if args.model in ["np", "anp", "cnp", "canp", "bnp", "banp", "tnpd", "tnpa", "tnpnd"]:

    # if args.model in ('bayesnpmog','bayescnpmog'): # TODO
    #     config['n_components'] = args.n_components

    # Hack to make work with MixtureOfDiagNormals
    args.is_fp64 = False
...
def train(args, model):
    ...
    if args.train_kernel == 'rbf':
        kernel = RBFKernel()
    elif args.train_kernel == 'rbf_fixed':
        kernel = RBFKernelFixedLengthscale()
    elif args.train_kernel == 'matern':
        kernel = Matern52Kernel()
    elif args.train_kernel == 'matern_fixed':
        kernel = Matern52KernelFixedLengthscale()
    elif args.train_kernel == 'periodic':
        kernel = PeriodicKernel()
    elif args.train_kernel == 'weakly_periodic':
        kernel = WeaklyPeriodicKernel()
    elif args.train_kernel == 'weakly_periodic_fixed':
        kernel = WeaklyPeriodicKernelFixedLengthscale()
    elif args.train_kernel == 'mixsine':
        kernel = 'mixsine'
    else:
        raise ValueError(f'Invalid kernel {args.train_kernel}')

    sampler = GPSampler(kernel, seed=args.train_seed)
...
def gen_evalset(args):
    if args.eval_kernel == 'rbf':
        kernel = RBFKernel()
    elif args.eval_kernel == 'rbf_fixed':
        kernel = RBFKernelFixedLengthscale()
    elif args.eval_kernel == 'matern':
        kernel = Matern52Kernel()
    elif args.eval_kernel == 'matern_fixed':
        kernel = Matern52KernelFixedLengthscale()
    elif args.eval_kernel == 'periodic':
        kernel = PeriodicKernel()
    elif args.eval_kernel == 'weakly_periodic':
        kernel = WeaklyPeriodicKernel()
    elif args.eval_kernel == 'weakly_periodic_fixed':
        kernel = WeaklyPeriodicKernelFixedLengthscale()
    elif args.eval_kernel == 'mixsine':
        kernel = 'mixsine'
    else:
...
def eval(args, model):
    ...
    if args.mode == 'eval':
        ...
        if args.eval_logfile is None:
            ...
            if args.t_noise is not None:
                eval_logfile += f'_s{args.max_num_cavi_steps}_tn_{args.t_noise}'
            eval_logfile += '.log'
        else:
...
def plot(args, model):
    ...
    with torch.no_grad():
        if args.model in ["np", "anp", "bnp", "banp", "npsa", "bayesnpmog"]:
            ...
            outs = model(batch, num_samples=num_smp, reduce_ll=False, max_n_steps=args.max_num_cavi_steps, min_n_steps=args.min_num_cavi_steps)
        else:
            outs = model(batch, reduce_ll=False)
        
        tar_loss = outs.tar_ll  # [Ns,B,Nt] ([B,Nt] for CNP)
        
        if args.model in ["cnp", "canp", "tnpd", "tnpa", "tnpnd", "bayescnp", "bayescnpmog"]:
        ...
        if args.model in ["np", "anp", "bnp", "banp", "tnpa", "tnpnd", "npsa", "bayesnpmog"]:
            ...
            pred = model.predict(batch.xc, batch.yc, xt, num_samples=num_smp, max_n_steps=args.max_num_cavi_steps, min_n_steps=args.min_num_cavi_steps)
        else:
